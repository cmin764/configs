...
| **Merge (per Claude profile)** | `.claude/user/settings.json` (installs to `<profile>/settings.json`) | Claude Code itself writes to this file (permission grants, plugin state). A plain copy on push would erase legitimate accumulated state; a plain copy on pull would drag that noise into the repo. Deep-merge, repo wins on conflicts, machine-only keys survive a push. Applied per profile, same set as the symlinks above. `model`, `effortLevel`, `modelSettings`, and `subagentPromptCacheTtl` are now template-owned (repo wins on push); the same repo-wins rule means a `--push` also re-applies `enabledPlugins.claude-mem: true` to any machine where it's currently disabled by hand -- informational, not a bug. |
...
7. **Per-org Claude Code auth**, if any work directory needs a full separate
   ...
   **Ports and data dirs isolate the data, not the billing.** Where a
   ...
   - **`ANTHROPIC_AUTH_TOKEN=<output of CLAUDE_CONFIG_DIR=~/.claude-<org>
     ...
     in the keychain. Also verified the failure side by corrupting the
     token: the SDK child hangs about 3 minutes, then the worker logs
     `SDK authentication failed ... 401 OAuth access token is invalid` and
     parks the batch -- no fallback to the keychain, no observations, and
